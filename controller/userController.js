...
const createUser = asyncHandler(async (req, res) => {
  const { firstname, lastname, mobile, email, password, cpassword } = req.body;
  ...
  if (password !== cpassword) {
    return res.status(400).json({
      status: 400,
      message: "Password and current password not match.",
    });
  }
  try {
...
const loginUrlCtrl = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (findUser && (await findUser.isPasswordMatched(password))) {
      // Generate refresh token and update user with it
...
const getallUser = asyncHandler(async (req, res) => {
  try {
    ...
    res.status(200).json({
      status: 200,
      data: getUsers,
      message: "Show all User List successfully",
    });
  } catch (err) {
    ...
    res.status(500).json({
      status: 500,
      message: "Sorry! User isn't showing.",
    });
  }
});
